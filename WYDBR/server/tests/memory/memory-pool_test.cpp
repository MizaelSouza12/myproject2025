/**
 * @file memory-pool_test.cpp
 * @brief Testes unitários para o componente memory-pool
 */
...
#include "memory/memory-pool.h"

/**
 * @brief Classe de teste para memory-pool
 */
class Memory-poolTest {
public:
    /**
     ...
     */
    bool runAllTests() {
        std::cout << "Executando testes para memory-pool..." << std::endl;
        ...
        if (result) {
            std::cout << "✓ Todos os testes de memory-pool passaram com sucesso!" << std::endl;
        } else {
            std::cerr << "✗ Falha nos testes de memory-pool!" << std::endl;
        }
...
private:
    ...
    bool testBasicInitialization() {
        ...
        wydbr::memory::Memory-pool instance;
