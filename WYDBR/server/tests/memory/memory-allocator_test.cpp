/**
 * @file memory-allocator_test.cpp
 * @brief Testes unitários para o componente memory-allocator
 */
...
/**
 * @brief Classe de teste para memory-allocator
 */
class Memory-allocatorTest {
public:
    /**
     ...
     */
    bool runAllTests() {
        std::cout << "Executando testes para memory-allocator..." << std::endl;
        ...
        if (result) {
            std::cout << "✓ Todos os testes de memory-allocator passaram com sucesso!" << std::endl;
        } else {
            std::cerr << "✗ Falha nos testes de memory-allocator!" << std::endl;
        }
...
private:
    ...
    bool testBasicInitialization() {
        ...
        wydbr::memory::Memory-allocator instance;
